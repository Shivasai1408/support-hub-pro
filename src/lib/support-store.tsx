...
export function SupportProvider({ children }: { children: ReactNode }) {
  ...
  const [user, setUser] = useState<SupportContextValue["user"]>(() => { try { const raw = localStorage.getItem("support-hub-user"); if (!raw) return null; const parsed = JSON.parse(raw); if (parsed?.email === "maya@helm.support" || parsed?.name === "Maya Okonkwo") { localStorage.removeItem("support-hub-user"); return null; } return parsed; } catch { return null; } });
